...
const JobsSection = () => {
  ...
  // Fixed search functionality with date validation
  const filteredJobs = jobs.filter(job => {
    // First check if the job deadline has passed
    if (job.lastDateToApply) {
      ...
      // If deadline has passed, don't show this job
  ...
  const handleApplyClick = (job) => {
    setSelectedJob({ title: job.title, id: job._id });
    setIsModalOpen(true);
  };
        ...
        <div className="mb-8 px-4 sm:px-6 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 sm:gap-6">
          ...
          <div className="flex items-center bg-blue-50 dark:bg-gray-700 rounded-lg px-4 py-2">
            <span className="font-medium text-blue-600 dark:text-blue-300">
              Total Open Positions: <span className="font-bold">{filteredJobs.length}</span>
            </span>
          </div>
        </div>
